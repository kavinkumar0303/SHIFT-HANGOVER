...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql://postgres.rfeahsofnkcobbsowegx:kavinkumar0303_@aws-0-ap-southeast-2.pooler.supabase.com:6543/postgres"
)
SQLITE_DB_PATH = os.getenv("SQLITE_DB_PATH", "database/shift_handover.db")
...
def seed_data_from_json(
    tickets_json_path: str = "data/tickets.json",
    incidents_json_path: str = "data/incidents.json"
) -> Tuple[int, int]:
    """
...
def get_all_activities_from_db() -> List[Dict[str, Any]]:
    """Retrieves combined tickets and incidents from the database."""
    return get_all_tickets() + get_all_incidents()
...
def get_handover_reports(limit: int = 50) -> List[Dict[str, Any]]:
...
def get_handover_report_by_id(report_id: int) -> Optional[Dict[str, Any]]:
...
def get_database_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            db_type = "PostgreSQL (Supabase Cloud)" if is_pg else "SQLite (Local Fallback)"
            db_target = "aws-0-ap-southeast-2.pooler.supabase.com:6543/postgres" if is_pg else SQLITE_DB_PATH
        ...
        return {
            "connected": False,
            "engine": "PostgreSQL (Supabase Cloud)" if is_postgres() else "SQLite",
            "error": str(e),
            "tickets_count": 0,
            "incidents_count": 0,
            "reports_count": 0,
